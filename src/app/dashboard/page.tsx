...
import { Situation, PublicSituation, PaginatedResponse } from '@/types'
...
export default function Dashboard() {
  ...
  const [publicSituations, setPublicSituations] = useState<PublicSituation[]>([])
  ...
  const fetchPublicSituations = async () => {
    setIsDiscoverLoading(true)
    try {
      const response = await api.get<PaginatedResponse<PublicSituation>>(
        `/discover/situations?page=${discoverPage}&per_page=12`
      )
      setPublicSituations(response.data.data || [])
      setDiscoverTotalPages(response.data.total_pages || 1)
    } catch (err) {
      console.error(err)
    } finally {
          ...
          <>
            ...
            {isDiscoverLoading ? (
              ...
              <>
                {/* Grid */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {publicSituations.map((situation, index) => (
                    <div
                      key={situation.id}
